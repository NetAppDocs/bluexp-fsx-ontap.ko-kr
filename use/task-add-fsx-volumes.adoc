...
.. 최적화(FlexGroup)를 선택한 경우 볼륨 데이터를 분산할 구성요소의 수를 지정해야 합니다. 데이터 배포를 위해 같은 수의 구성 요소라도 사용하는 것이 좋습니다. 모범 사례는 8개 구성 요소입니다.
+
image:screenshot-constituents.png["볼륨 데이터를 분산할 구성요소의 수를 지정하는 필드의 스크린샷"]
